  ...
  public extractDataFromResponse(data, dataPath, attr = null) {
    if (!data || !dataPath) {
      ...
      data.map(function(v,k){
        if(v.$class == "org.acme.hacknetwork.CoToCoTransaction"){
          v.company1 = v.company1.split('#')[1]
          v.company2 = v.company2.split('#')[1]
        }
        if(v.$class == "org.acme.hacknetwork.CompanyParticipant"){
          v.bank = v.bank.split('#')[1]
        }
      })
      //if((data).split('#')[1]) data = (data).split('#')[1];
      return data;
    }
